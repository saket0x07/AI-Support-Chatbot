...
1. **Navigate to Settings > Billing dashboard.** - Navigate to Settings > Billing dashboard.
2. **Click on the 'Manage Billing via Stripe' link.** - Click on the 'Manage Billing via Stripe' link.
3. **Modify billing details in the secure Stripe-managed portal interface.** - Modify billing details in the secure Stripe-managed portal interface.
4. **Return to the primary workspace once modifications are completed.** - Return to the primary workspace once modifications are completed.
...
### Q: Is the portal interface secure?
**A**: Yes, the portal is hosted entirely on Stripe's infrastructure, utilizing high-grade TLS protection and verified tokens.

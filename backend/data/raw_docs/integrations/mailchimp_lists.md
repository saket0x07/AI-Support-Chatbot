...
1. **Connect Mailchimp API account token.** - Connect Mailchimp API account token.
2. **Choose target audience email list.** - Choose target audience email list.
3. **Map contact profile parameters (e.g. Email, First Name).** - Map contact profile parameters (e.g. Email, First Name).
4. **Activate trigger logs to push contacts.** - Activate trigger logs to push contacts.
...
### Q: Can I assign tags to contacts on sync?
**A**: Yes, define tag values inside the integration module settings panel.

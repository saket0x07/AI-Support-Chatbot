...
1. **Log into Shopify Admin and navigate to Settings > Notifications.** - Log into Shopify Admin and navigate to Settings > Notifications.
2. **Scroll to Webhooks and click Create Webhook.** - Scroll to Webhooks and click Create Webhook.
3. **Select event triggers (e.g. Order Creation).** - Select event triggers (e.g. Order Creation).
4. **Enter your CloudFlow webhook callback destination address.** - Enter your CloudFlow webhook callback destination address.
5. **Confirm version matching to authorize active sync.** - Confirm version matching to authorize active sync.
...
### Q: How does CloudFlow resolve order duplicates?
**A**: We check Shopify order UUID parameters to block duplicate executions.

## Related Articles

- [Stripe Payment Integration](../integrations/stripe_payment_integration.md)

...
1. **Authorize Trello connection in integrations board.** - Authorize Trello connection in integrations board.
2. **Select destination board ID and list name.** - Select destination board ID and list name.
3. **Set mapping rules** map ticket creation details to Trello card parameters.
4. **Run a test flow iteration.** - Run a test flow iteration.
...
**A**: Yes, map team member emails inside the Trello action properties window.

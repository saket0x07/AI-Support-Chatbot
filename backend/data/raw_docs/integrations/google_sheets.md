...
1. **Authenticate Google integration services.** - Authenticate Google integration services.
2. **Select target spreadsheet filename and sheet tab.** - Select target spreadsheet filename and sheet tab.
3. **Map workflow variables to sheet columns (e.g. Column A, Column B).** - Map workflow variables to sheet columns (e.g. Column A, Column B).
4. **Test data pipeline flow.** - Test data pipeline flow.
...
### Q: Are sheet modifications real-time?
**A**: Yes, updates are executed asynchronously on cell triggers immediately.

## Related Articles

- [Notion Pages Synchronization](../integrations/notion_pages_synchronization.md)
- [PostgreSQL Database Connector](../integrations/postgresql_database_connector.md)

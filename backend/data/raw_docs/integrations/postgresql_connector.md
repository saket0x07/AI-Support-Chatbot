...
1. **Configure SQL step node in workflow model.** - Configure SQL step node in workflow model.
2. **Provide host endpoint address, database name, and login credentials.** - Provide host endpoint address, database name, and login credentials.
3. **Write the INSERT/UPDATE statement template using workflow keys.** - Write the INSERT/UPDATE statement template using workflow keys.
4. **Verify network access and test integration connectivity.** - Verify network access and test integration connectivity.
...
### Q: Do you persist database passwords?
**A**: Credential values are encrypted at rest using AES-256 before storage.

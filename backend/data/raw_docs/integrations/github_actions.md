...
1. **Create a GitHub workflow yaml file inside your repository.** - Create a GitHub workflow yaml file inside your repository.
2. **Define deployment steps using our official action script** `uses
3. **Add repository secrets** `CLOUDFLOW_API_KEY` and `CLOUDFLOW_FLOW_ID`.
4. **Commit code changes to execute and test integration sync.** - Commit code changes to execute and test integration sync.
...
### Q: Can GitHub trigger workflow runs in private workspaces?
**A**: Yes, keys with appropriate developer scopes permit action executions across private repositories.

## Related Articles

- [API Access Key Generation](../integrations/api_access_key_generation.md)

...
1. **Access Developer settings and select the 'Telemetry' tab.** - Access Developer settings and select the 'Telemetry' tab.
2. **Set charting filters (e.g. last 24 hours).** - Set charting filters (e.g. last 24 hours).
3. **Inspect graphs for latency changes or error distributions.** - Inspect graphs for latency changes or error distributions.
...
### Q: Can I pipe API telemetry into external monitoring tools?
**A**: Yes, Datadog and Prometheus sync options are available on Professional and Enterprise plans.

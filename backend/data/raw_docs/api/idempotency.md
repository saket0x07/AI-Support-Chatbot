...
1. **Add the header `Idempotency-Key` along with a unique UUID string.** - Add the header `Idempotency-Key` along with a unique UUID string.
2. **Execute the POST request.** - Execute the POST request.
3. **If network fails, retry the request with the identical key.** - If network fails, retry the request with the identical key.
4. **The server returns the cached response, preventing duplicated actions.** - The server returns the cached response, preventing duplicated actions.
...
### Q: How long are idempotency records cached?
**A**: Idempotency responses are cached for 24 hours from the initial request.

## Related Articles

- [API Error Codes Reference](../api/api_error_codes_reference.md)
- [Custom API Headers Reference](../api/custom_api_headers_reference.md)

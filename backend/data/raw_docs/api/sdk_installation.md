...
1. **Python** Run `pip install cloudflow-sdk`.
2. **Node.js** Run `npm install @cloudflow/sdk`.
3. **Initialize the client instance** `const client = new CloudFlowClient({ apiKey
4. **Initiate a test run call to verify connection status.** - Initiate a test run call to verify connection status.
...
### Q: Are the SDK libraries open source?
**A**: Yes, all our SDK repositories are hosted on GitHub under MIT licenses.

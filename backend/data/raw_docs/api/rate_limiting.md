...
1. **Review rate limit headers in API response metadata.** - Review rate limit headers in API response metadata.
2. **Read** `X-RateLimit-Limit` (quota allowed), `X-RateLimit-Remaining` (remaining counts), `X-RateLimit-Reset` (epoch time of reset).
3. **Implement client-side exponential backoff handling to delay retry requests when receiving 429 status codes.** - Implement client-side exponential backoff handling to delay retry requests when receiving 429 status codes.
...
**Resolution**: Check your rate limits: Starter plans allow 60 requests/minute, Professional plans permit 1,000 requests/minute.

## Frequently Asked Questions

### Q: Do webhook runs count toward rate limits?
**A**: No, webhooks processed by CloudFlow are excluded from standard API request throttling allocations.

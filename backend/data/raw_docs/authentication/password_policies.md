...
1. **Log in as Owner and go to Organization settings > Security Policies.** - Log in as Owner and go to Organization settings > Security Policies.
2. **Find the 'Password Complexity Enforcement' section.** - Find the 'Password Complexity Enforcement' section.
3. **Toggle 'Advanced Complexity' to active status.** - Toggle 'Advanced Complexity' to active status.
4. **Define values** Minimum length (e.g., 12 characters), special characters count, digits count, and uppercase count.
5. **Check 'Enforce Password Expiry' and specify the frequency in days (e.g., 90 days).** - Check 'Enforce Password Expiry' and specify the frequency in days (e.g., 90 days).
6. **Save configuration settings.** - Save configuration settings.
...
### ⚠️ Users locked out during policy migration
**Description**: Encountered when system bounds or client validations fail.
**Resolution**: When enabling new policies, active sessions remain authenticated. Users are only forced to update passwords at their next login event.

## Frequently Asked Questions

### Q: Does CloudFlow block common passwords?
**A**: Yes, we integrate with HaveIBeenPwned API to automatically reject compromised or weak password strings.

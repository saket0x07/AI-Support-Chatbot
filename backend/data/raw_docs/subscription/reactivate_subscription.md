...
1. **Log in to the dashboard.** - Log in to the dashboard.
2. **Select 'Reactivate Plan' on the suspension warning overlay.** - Select 'Reactivate Plan' on the suspension warning overlay.
3. **Settle outstanding invoices using a primary payment card.** - Settle outstanding invoices using a primary payment card.
4. **Wait 30 seconds for the system status updater to return your workspace to active state.** - Wait 30 seconds for the system status updater to return your workspace to active state.
...
### Q: Will my paused flows resume running automatically?
**A**: No, for safety reasons, workflows paused during suspension must be reactivated manually using the flow toggle switch.

## Related Articles

- [Cancel Subscription Policy](../subscription/cancel_subscription_policy.md)
- [Managing Card Declines](../subscription/managing_card_declines.md)

...
1. **Check destination HTTPS server SSL validity.** - Check destination HTTPS server SSL validity.
2. **Ensure root certificates are updated.** - Ensure root certificates are updated.
...
### Q: Can I bypass SSL validation in test environments?
**A**: Yes, you can toggle off 'Strict SSL Verification' under request card security parameters.

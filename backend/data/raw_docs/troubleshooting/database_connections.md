...
1. **Check connection strings values.** - Check connection strings values.
2. **Verify database username/password have not expired.** - Verify database username/password have not expired.
3. **Ensure max pool size parameter is set high enough.** - Ensure max pool size parameter is set high enough.
...
**A**: PostgreSQL, MySQL, SQL Server, MongoDB, and SQLite connectors are supported natively.

## Related Articles

- [PostgreSQL Database Connector](../troubleshooting/postgresql_database_connector.md)

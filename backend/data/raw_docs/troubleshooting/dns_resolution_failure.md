...
1. **Verify external hostname address syntax.** - Verify external hostname address syntax.
2. **Run nslookup commands to verify IP visibility.** - Run nslookup commands to verify IP visibility.
3. **Ensure destination server dns updates have finished propagating.** - Ensure destination server dns updates have finished propagating.
...
### Q: Do custom domains resolve inside flows?
**A**: Yes, we utilize global Google DNS servers to resolve hostnames.

## Related Articles

- [Investigating Missing Triggers](../troubleshooting/investigating_missing_triggers.md)
- [SSL Certificate Failures](../troubleshooting/ssl_certificate_failures.md)

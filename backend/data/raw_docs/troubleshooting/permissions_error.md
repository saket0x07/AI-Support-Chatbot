...
1. **Verify active user profile role settings.** - Verify active user profile role settings.
2. **Confirm group permissions map parameters.** - Confirm group permissions map parameters.
3. **Request workspace administrator validation if roles need upgrades.** - Request workspace administrator validation if roles need upgrades.
...
### Q: Can Developers generate billing reports?
**A**: No, only Billing managers and Owners have permissions to compile billing logs.

## Related Articles

- [Role-Based Access Control Mapping](../troubleshooting/role_based_access_control_mapping.md)
- [Role-Based Access Control Mapping](../troubleshooting/role_based_access_control_mapping.md)

...
1. **Go to Settings > Connected Applications.** - Go to Settings > Connected Applications.
2. **Locate the failing integration card and click 'Reconnect'.** - Locate the failing integration card and click 'Reconnect'.
3. **Re-authorize access in the external application prompt screen.** - Re-authorize access in the external application prompt screen.
...
### Q: Why do credentials expire regularly?
**A**: Some platforms enforce a token expiration time policy of 30 days for third-party integrations.

## Related Articles

- [Token Lifecycle Management](../troubleshooting/token_lifecycle_management.md)
- [Configuring OAuth App Authorizations](../troubleshooting/configuring_oauth_app_authorizations.md)
